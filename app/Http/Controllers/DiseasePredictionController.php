    ...
    public function diagnose(Request $request)
{
    ...
    // Calculate confidence based on closest match
    $bestMatchCount = 0;
    ...
    foreach ($formattedData as $entry) {
        ...
        $matchingSymptoms = array_intersect_key($entrySymptoms, $selectedSymptoms);

        if (count($matchingSymptoms) > $bestMatchCount) {
            $bestMatchCount = count($matchingSymptoms);
        }
    }

    $confidence = $totalSelected > 0 ? round(($bestMatchCount / $totalSelected) * 100, 2) : 0;
...
public function evaluateModel()
{
    ...
    if (($handle = fopen(storage_path('app/dataset/Training.csv'), 'r')) !== false) {
        $header = fgetcsv($handle);
        $attributes = array_filter($header, fn($col) => strtolower($col) !== 'prognosis');
        ...
        while (($row = fgetcsv($handle)) !== false) {
            ...
            foreach ($attributes as $attr) {
                $formatted[$attr] = (int) $record[$attr]; // ensure numeric 0/1
            }
            $formatted['Disease'] = $record['prognosis'];

            $trainingData[] = $formatted;
        }
        fclose($handle);
    }

    if (empty($trainingData)) {
        return response()->json(['error' => 'No training data found'], 404);
    }

    // === Step 2: Train the ID3 decision tree ===
    $tree = $this->id3->train($trainingData, $attributes);
    ...
    if (($handle = fopen(storage_path('app/dataset/Testing.csv'), 'r')) !== false) {
        ...
        while (($row = fgetcsv($handle)) !== false) {
            ...
            $formatted = [];
            foreach ($attributes as $attr) {
                $formatted[$attr] = (int) $record[$attr];
            }
            $formatted['Disease'] = $record['prognosis'];

            $testingData[] = $formatted;
        }
        fclose($handle);
    }

    if (empty($testingData)) {
        return response()->json(['error' => 'No testing data found'], 404);
    }
    ...
    foreach ($testingData as $index => $sample) {
        $actual = $sample['Disease'];
        $predicted = $this->id3->classify($tree, $sample);
        ...
        $results[] = [
            'case' => $index + 1,
            'symptoms' => implode(", ", array_keys(array_filter($sample, fn($v, $k) => $v == 1 && $k !== 'Disease', ARRAY_FILTER_USE_BOTH))),
            'actual' => $actual,
            'predicted' => $predicted,
            'correct' => $isCorrect ? '✅' : '❌'
        ];
    ...
    // === Step 5: Accuracy ===
    $accuracy = ($correct / count($testingData)) * 100;

    return response()->json([
        'accuracy' => round($accuracy, 2),
        'total_cases' => count($testingData),
        'results' => $results
    ]);
}
